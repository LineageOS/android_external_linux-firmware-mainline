			...
			for entry_name, entry in entries.items():
				...
				for _, blobs in entry.license_to_blobs.items():
					for blob in blobs:
						...
						all_entry_targets.add(target_name)
				...
				if all_entry_targets:
					# Make a phony target that copies all entry files and symlinks
					f.write(f'\n')
					f.write(f'phony {{\n')
					f.write(f'    name: "{entry_aosp_target_name}",\n')
					f.write(f'    required: [\n')
					for symlink_target in sorted(all_entry_targets):
						f.write(f'        "{symlink_target}",\n')
					f.write(f'    ],\n')
					f.write(f'    visibility: ["//external/linux-firmware-mainline:__subpackages__"],\n')
					f.write(f'}}\n')

					all_first_level_targets.add(entry_aosp_target_name)
